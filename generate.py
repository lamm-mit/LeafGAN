...
#Edits by Markus Buehler, Sabrina Shen (MIT/LAMM) 2022
...
#MJB added
def circularloop_spiral(numberrotations, d_inc_rotation, nf, d, seed, seeds):
    r = d/2
    ...
    print ("Spiral rotations, increment every turn by ",d_inc_rotation,    " and do ", numberrotations,    "rotations")
    for rotats in  (range (numberrotations)):
     current_pos = 0.0
     #print ("rotation= ", rotats)
     iuu=0 # counter for current loop around
     while(current_pos < 1.0):
        #every step slightly increase radius. Once full rotation has been done, was increased by d_inc_rotation
        r=r+d_inc_rotation*step 

        print ("rotation= ", rotats, " current substep: ", iuu, " current radius: ", r, " current position: ", current_pos)
        #print ("latent collection: ", len(zs)    )
...
#MJB added
def point(x_s, y_s, xfactor, yfactor, seeds,frames,easing):
    
    # radius
    #r =  np.sqrt (x_s*x_s*xfactor*xfactor + y_s*y_s*yfactor*yfactor) 
    #1=1 full rotation 
    #current_pos = np.arctan2 (x_s*xfactor, y_s*yfactor)+
    
    zs = []
    # hardcoding in 512, prob TODO fix needed
    # latents_c = rnd.randn(1, G.input_shape[1])
    if(seeds is None):
        print ("Warning: Seed points to span coordinate system not specified!")
        rnd = np.random
        latents_a = rnd.randn(1, 512)
        latents_b = rnd.randn(1, 512)
        latents_c = rnd.randn(1, 512)
    ...
    if(len(x_s) is not 2):
        assert('Must choose exactly 2 coordinates!')
    
    print ("x_s, y_s, xfactor, yfactor: ", x_s, y_s, xfactor, yfactor, "number of frames: ", frames)
    ...
    # FIRST point 
    latents_x =  x_s[0]*xfactor
    latents_y = y_s[0]*yfactor
    
    #print ("radius int= ", radius)

    latents1 = latents_a + latents_x * latents_axis_x + latents_y * latents_axis_y

    # SECOND point
    latents_x =  x_s[1]*xfactor
    latents_y = y_s[1]*yfactor
    
    #print ("radius int= ", radius)

    latents2 = latents_a + latents_x * latents_axis_x + latents_y * latents_axis_y
    
    zs.append(latents1)
    zs.append(latents2)
    
    points = line_interpolate(zs,frames,easing)
    
    #return zs
    return points

def point_CSV(csvfile, seeds,frames,easing):
    
    print ("Generate from CSV file - read all data: ", csvfile)
    from numpy import genfromtxt
    pointdata = genfromtxt(csvfile, delimiter=',',skip_header=1)
    print (pointdata)
    
    #x_s=.9,0. --y_s=0.,0.9
    
    print (pointdata.shape[0])
    ...
    latents_axis_x = (latents_b - latents_a).flatten() #/ linalg.norm(latents_b - latents_a)
    latents_axis_y = (latents_c - latents_a).flatten() #/ linalg.norm(latents_c - latents_a)

    print ("norm a-b: ", linalg.norm(latents_a - latents_b), "norm a-c: ", linalg.norm(latents_a - latents_c))  
    
    for ii in range (pointdata.shape[0]):
        
    
         xs=pointdata[ii,0] 
         #x_s[1]=pointdata[ii+1,0]
         ys=pointdata[ii,1] 
         #y_s[1]=pointdata[ii+1,1]
        
         print ("x_s, y_s: ", xs, ys, "ii= ", ii, "number of frames: ", frames)
         ...
         # FIRST point 
         #latents_x =  xs *xfactor
         #latents_y = ys*yfactor
    
         #print ("radius int= ", latents_x)

         latents1 = latents_a + xs * latents_axis_x + ys * latents_axis_y
         
         #Store FIRST point ... will be inserted later
         if ii==0:
             firstpoint=latents1
    ...
    if frames==0:
        print ("no interpolation, frames==0")
        return zs
    if frames>0:
        #insert FIRST entry (start point)
        print ("Insert last point...", len (points))
        points.append(latents1)
        print ("use interpolation, frames==", frames)
        #return zs
        return points
...
def float_range(s: str) -> List[int]:
    '''Accept either a comma separated list of numbers 'a,b,c' or a range 'a-c' and return as a list of floats.'''
...
def interpolate(G,device,projected_w,seeds,random_seed,space,truncation_psi,label,frames,noise_mode,outdir,interpolation,easing,diameter=None,start=None,stop=None, numberrotations=None, d_inc_rotation=None, x_s=None, y_s=None, xfactor=None, yfactor=None, csvfile=None):
    if(interpolation=='noiseloop' or interpolation=='circularloop' or interpolation=='circularloop_spiral' or interpolation=='2Dpoint'or interpolation=='2Dpoint_CSV'):
        ...
        elif(interpolation=='circularloop_spiral'): 
            #numberrotations = 5 #later read via function call
            #d_inc_rotation = 500
            print ("Number of rotations: ", numberrotations," Increments each rotation: ", d_inc_rotation)
            points=  circularloop_spiral(numberrotations, d_inc_rotation, frames, diameter, random_seed, seeds)

        elif(interpolation=='2Dpoint'): 
            #numberrotations = 5 #later read via function call
            #d_inc_rotation = 500
            print ("2D point generation (two points)")
            points= point (x_s, y_s, xfactor, yfactor,  seeds, frames, easing)  
        elif(interpolation=='2Dpoint_CSV'): 
            #numberrotations = 5 #later read via function call
            #d_inc_rotation = 500
            print ("2D point generation (read multiple points from CSV file)")
            points= point_CSV (csvfile,  seeds, frames, easing)
            print(points)
...
@click.option('--numberrotations', type=int, help='for circular_spiral option, how many rotations are done', default= 1, show_default=True)
@click.option('--d_inc_rotation', type=float, help='for circular_spiral option, increment of radius at each rotation', default= 10.0, show_default=True)       
        
@click.option('--x_s', type=float_range, help='x value (in scaled coordinates) - for point generation')#, default="0.0,0.0", show_default=True)
@click.option('--y_s', type=float_range, help='y value (in scaled coordinates) - for point generation')#, default="0.0,0.0", show_default=True)  

@click.option('--xfactor', type=float, help='x factor value - for point generation', default= 1.0, show_default=True)     
@click.option('--yfactor', type=float, help='y factor value - for point generation', default= 1.0, show_default=True)             
    
@click.option('--csvfile', type=str, help='CSV file - for point generation - interpolation=2Dpoint_CSV')     
...
@click.option('--interpolation', type=click.Choice(['linear', 'slerp', 'noiseloop', 'circularloop', 'circularloop_spiral', '2Dpoint', '2Dpoint_CSV']), default='linear', help='interpolation type', required=True)
...
def generate_images(
    ...
    stop: Optional[float],
    numberrotations: Optional[int],
    d_inc_rotation: Optional[float],
    x_s: Optional [List[float]],
    y_s: Optional [List[float]],
    xfactor: Optional [float],
    yfactor: Optional [float],
    ...
    if(process=='image'):
        ...
        elif(interpolation=='noiseloop' or 'circularloop' or 'circularloop_spiral' or '2Dpoint' or '2Dpoint_CSV'):
        ...
        elif process=='interpolation' and interpolation=='circularloop_spiral':
            interpolate(G,device,projected_w,seeds,random_seed,space,truncation_psi,label,frames,noise_mode,dirpath,interpolation,easing,diameter,numberrotations=numberrotations,d_inc_rotation=d_inc_rotation)
        elif process=='interpolation' and interpolation=='2Dpoint':
            interpolate(G,device,projected_w,seeds,random_seed,space,truncation_psi,label,frames,noise_mode,dirpath,interpolation,easing,x_s=x_s, y_s=y_s, xfactor=xfactor, yfactor=yfactor)            
        elif process=='interpolation' and interpolation=='2Dpoint_CSV':
            print ("Generate from CSV file...")
            interpolate(G,device,projected_w,seeds,random_seed,space,truncation_psi,label,frames,noise_mode,dirpath,interpolation,easing,csvfile=csvfile, xfactor=xfactor, yfactor=yfactor)
            print ("Generate from CSV file...")
